...
describe('Critical Flow Regressions', () => {
  ...
  it('Flow 2: Tracker Progression -> Carbon World Level Progression', () => {
    ...
    const state0 = createCarbonWorldState({ completedActions: initialState.completedActionIds.length, totalActions: 3 });
    expect(state0.stage).toBe('seed');
    expect(state0.progressPercent).toBe(0);

    // Complete 1 action
    const tState1 = toggleTrackedAction(initialState, 'act-1');
    const state1 = createCarbonWorldState({ completedActions: tState1.completedActionIds.length, totalActions: 3 });
    expect(state1.stage).toBe('sprout');
    expect(state1.progressPercent).toBe(33);

    // Complete 2nd action
    const tState2 = toggleTrackedAction(tState1, 'act-2');
    const state2 = createCarbonWorldState({ completedActions: tState2.completedActionIds.length, totalActions: 3 });
    expect(state2.stage).toBe('garden');
    expect(state2.progressPercent).toBe(67);

    // Complete 3rd action
    const tState3 = toggleTrackedAction(tState2, 'act-3');
    const state3 = createCarbonWorldState({ completedActions: tState3.completedActionIds.length, totalActions: 3 });
    ...
    const stateUndo = createCarbonWorldState({ completedActions: tStateUndo.completedActionIds.length, totalActions: 3 });
